...
function initLoginForm() {
  ...
  eyeLCoords = { x: svgCoords.x + 85.5, y: svgCoords.y + 78.5 };
  eyeRCoords = { x: svgCoords.x + 114.5, y: svgCoords.y + 78.5 };
...
try {
  ...
  if (saved) {
    ...
    buildMenu(auth.user?.permissions || [], auth.user?.roles || []);
  }
} catch (_) {}
...
form.addEventListener("submit", async (e) => {
  ...
  if (res.ok) {
    ...
    buildMenu(data.user.permissions, data.user.roles);
  } else {
    alert(data.error || "Login failed");
  }
});

function buildMenu(perms, roles = []) {
  menu.innerHTML = "";
  // Group permissions by module
  const byModule = perms.reduce((acc, p) => {
    if (!acc[p.module]) acc[p.module] = new Set();
    acc[p.module].add(p.name_permission);
    return acc;
  }, {});

  // Orders module buttons
  const orderPerms = byModule["Order"] || new Set();
  if (orderPerms.has("View"))
    addMenuLink("List Orders", "/list-orders.html", "bi-list-ul", "primary");
  if (orderPerms.has("Add"))
    addMenuLink(
      "Create Order",
      "/create-order.html",
      "bi-plus-circle",
  ...
  if (orderPerms.has("Delete"))
    addMenuLink("Delete Order", "/delete-order.html", "bi-trash", "danger");

  // Customers module buttons
  const customerPerms = byModule["Customer"] || new Set();
  if (customerPerms.has("View"))
    addMenuLink("Customers", "/list-customers.html", "bi-people", "info");
  if (customerPerms.has("Add"))
    addMenuLink(
      "New Customer",
  ...
  if (customerPerms.has("Delete"))
    addMenuLink(
      "Delete Customer",
      "/delete-customer.html",
      "bi-trash",
      "danger"
    );

  if (Array.isArray(roles) && roles.includes("Admin")) {
    addMenuLink("Admin Dashboard", "/admin.html", "bi-shield-lock", "dark");
  }
}

function addMenuLink(label, href, icon, variant) {
  const a = document.createElement("a");
  const variants = {
    primary: "btn btn-primary shadow-sm",
    success: "btn btn-success shadow-sm",
    warning: "btn btn-warning shadow-sm",
    danger: "btn btn-danger shadow-sm",
    dark: "btn btn-dark shadow-sm",
    info: "btn btn-info shadow-sm",
    "success-outline": "btn btn-outline-success shadow-sm",
  };
  a.className = variants[variant] || "btn btn-secondary shadow-sm";
  a.href = href;
  a.style.cssText = "text-decoration:none; margin: 0.25rem;";
  a.innerHTML = `<i class="bi ${icon} me-2"></i>${label}`;
  menu.appendChild(a);
}

function addButton(label, handler, variant = "default") {
  const btn = document.createElement("button");
  btn.textContent = label;
  btn.onclick = handler;
  btn.className = "btn";
  if (variant === "primary") btn.classList.add("btn-primary");
  if (variant === "danger") btn.classList.add("btn-danger");
...
async function loadOrders() {
  ...
  if (res.ok) {
    ...
    content.textContent = data.error || "Error";
  }
}
...
function showForm(action) {
  ...
  btn.onclick = async () => {
    ...
    const body =
      action === "Delete" ? undefined : JSON.stringify({ item: input.value });
